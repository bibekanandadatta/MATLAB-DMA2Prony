...
% See also DMAGROUPISOTHERMS, DMABUILDMASTERCURVE.
    ...
    p.addRequired('referenceTemperature', @(x) isnumeric(x) && isscalar(x) && isfinite(x));
    p.addParameter('TemperatureTolerance', 0.5, @(x) isnumeric(x) && isscalar(x) && x > 0);
    p.addParameter('ReferenceTolerance', 1.0, @(x) isnumeric(x) && isscalar(x) && x >= 0);
    p.addParameter('NumberOfLevels', 10, @(x) isnumeric(x) && isscalar(x) && x >= 3 && x == round(x));
    p.addParameter('DropFirstPoint', true, @(x) islogical(x) && isscalar(x));
    ...
    shiftFactors.ReferenceTemperature_C = repmat(isotherms.Temperature_C(referenceIndex), nSets, 1);

end

function setData = getSet(data, setValue)
% Isolate one sweep and remove values that cannot be used in a log-frequency power-law fit.
...
function [log10aT, diagnostic] = fitShiftPair(refData, shiftData, numberOfLevels, dropFirst)
    ...
    diagnostic.ReferenceFrequencyAtLevel_Hz     = inversePowerLaw(levels, pRef);
    diagnostic.ShiftedFrequencyAtLevel_Hz       = inversePowerLaw(levels, pShift);
end
